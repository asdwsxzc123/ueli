...
import { ClipBoardHistorySetting, Settings } from "@common/Extensions/ClipBoardHistory";
...
export class ClipboardHistoryExtension implements Extension {
        ...
        this.eventSubscriber.subscribe("actionInvocationStarted", ({ action }: { action: SearchResultItemAction }) => {
            ...
            if (extensionId === this.id) {
                if (handlerId === 'copyToClipboard') {
                    ...
                    itemId && this.insertOrUpdateClipboardItem(itemId.split('-')[1])
    ...
    private getPrefix(): string {
        const obj = this.settingsManager.getValue<ClipBoardHistorySetting>(
            this.settingKey,
            { prefix: "copy" },
        );
    ...
    private getValue() {
        return this.settingsManager.getValue<ClipBoardHistorySetting>(
            this.settingKey,
            { prefix: "copy" }
        );
    }
